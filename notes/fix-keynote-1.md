...
Written 12 September 2026 against `review-keynote-1.md`.
An earlier pass had edited the partials and scripts but left no report, render or screenshots; this pass checked each of its changes, finished the rest, rendered, screenshotted all 29 slides plus both fallbacks at 1920 by 1080, ran `./scripts/lint.sh` (clean) and every `llmisms.md` grep.
...
2. "2026. Live". One figure, the September cumulative infections panel, 712 px tall. Vintage in the attribution (release `results-1699`, 9 September, data to SitRep 115 of 6 September). Sitrep page dropped from the deck.
...
12. Toy seeded on the exponential solution of the recursion (Euler-Lotka rate), so no dip; `keynote-renewal.png` regenerated from the same script; ojs cell mirrors it.
13. Log axis ticks 1, 10, 100, 1,000 with explicit domain; top margin added so the axis label clears the top tick; slider labels no longer wrap (`--label-width`, `--input-width`).
14. Both tails cut.
15. Bullet restating the footer dropped.
16. Bullets dropped, diagram kept.
17. Bullet dropped, diagram kept (per the steer to cut bullets, not figures).
18. "the term you cannot write down".
19. PINN bullets cut; Millevoi caveat in the attribution; slide is two bullets and the figure.
...
29 numbered slides: title, talk plan, five dividers, 21 content, thank you. Two hidden fallbacks.
A 6, B 3 (+1 hidden), C 6 (+1 hidden), D 4, E 2.
...
- Some bullets still render at three lines: agent definition, pull request gloss, both SitRep bullets, Camacho, UDE, OutbreakFlow, TabPFN-TS, authorship. Each carries a fact; cutting is his call.
- If the 90% interval is wanted back, add it to `research-history.md` section 2 first (release `results-1699` analysis page, 9,749 to 19,236).
- The UDE slide has two bullets; a third would need a sourced PINN line.
